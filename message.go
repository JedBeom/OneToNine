...
func messageHandler(w http.ResponseWriter, r *http.Request) {
	...
	case "순위":
		...
		*/
		...
		Db.Table("records").Order("score desc").Where("userkey = ?", post.Userkey).Select("userkey, nickname, score, row_number () over (order by score desc").First(&myRecord)
		if err != nil {
			log.Println(err)
		...
		content := fmt.Sprintf(scoreTemplate, myRecord.RowNumber, myRecord.Nickname, myRecord.Score)
